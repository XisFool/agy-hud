...
| `display.showTokenBar` | boolean | `true` | 显示 Token 进度条 |
| `display.showBreadcrumbs` | boolean | `true` | 显示面包屑路径 |
| `display.showGitBranch` | boolean | `true` | 显示 Git 分支名 |
| `display.breadcrumbCount` | number | `3` | 面包屑显示层级数 |
| `display.unicode` | boolean | `true` | 启用 Unicode 字符（自动检测，可强制覆盖） |
...
| `language` | string | `"auto"` | 界面语言（auto / zh / en） |

也可通过环境变量强制 Unicode 行为：
...
Windows 优先读取 Credential Manager，并把可用 token 写入短期 `agy-hud-token.json` 镜像供状态栏进程读取。其他平台和文件回退按优先级搜索以下路径：
